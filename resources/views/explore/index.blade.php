    ...
    <main class="mx-auto max-w-6xl px-5 py-8">
        ...
        @if(in_array($type, ['all', 'product', 'service'], true))
            <section class="mt-10"><div class="flex items-end justify-between"><div><p class="text-xs font-black uppercase tracking-[.16em] text-[#F97316]">Compra y contrata</p><h2 class="mt-1 text-2xl font-black">Productos y servicios</h2></div><span class="text-sm font-bold text-[#6B7D83]">{{ $listings->total() }} resultados</span></div>
                <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">@forelse($listings as $listing)<article class="rounded-[1.5rem] border border-[#123B4A]/10 bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-3"><span class="rounded-full bg-[#E9F7F0] px-3 py-1 text-xs font-black text-[#14734A]">{{ $listing->type->value === 'product' ? 'Producto' : 'Servicio' }}</span>@if($listing->stock !== null)<span class="text-xs font-bold text-[#6B7D83]">{{ $listing->stock }} disponibles</span>@endif</div><h3 class="mt-4 text-lg font-black">{{ $listing->name }}</h3><p class="mt-1 text-sm font-bold text-[#6B7D83]">{{ $listing->vendor->display_name }}</p><p class="mt-3 line-clamp-2 text-sm leading-6">{{ $listing->description }}</p><div class="mt-5 flex items-center justify-between gap-3"><strong>{{ $listing->price_amount !== null ? '$'.number_format($listing->price_amount / 100, 2).' MXN' : 'Cotización' }}</strong>@if($listing->type->value === 'product' && $listing->price_type->value === 'fixed')<a class="rounded-full bg-[#F97316] px-4 py-2 text-sm font-black text-white" href="{{ auth()->check() ? route('products.checkout', $listing) : route('login') }}">Comprar</a>@else<a class="rounded-full border border-[#123B4A]/10 px-4 py-2 text-sm font-black" href="{{ route('profile.show', $listing->vendor->user) }}">Ver perfil</a>@endif</div></article>@empty<p class="col-span-full rounded-2xl border border-dashed border-[#123B4A]/20 p-8 text-center font-bold text-[#6B7D83]">No encontramos ofertas con esos filtros.</p>@endforelse</div>
        ...
        @if(in_array($type, ['all', 'job_request'], true))
            <section class="mt-12 pb-16"><div class="flex items-end justify-between"><div><p class="text-xs font-black uppercase tracking-[.16em] text-[#F97316]">Oportunidades</p><h2 class="mt-1 text-2xl font-black">Solicitudes de la comunidad</h2></div><span class="text-sm font-bold text-[#6B7D83]">{{ $jobRequests->total() }} resultados</span></div><div class="mt-5 grid gap-4 sm:grid-cols-2">@forelse($jobRequests as $job)<article class="rounded-[1.5rem] border border-[#F97316]/15 bg-[#FFF9F3] p-5"><span class="text-xs font-black uppercase tracking-[.12em] text-[#D85B0B]">{{ $job->urgency === 'urgent' ? 'Urgente' : 'Solicitud activa' }}</span><h3 class="mt-3 text-lg font-black">{{ $job->title }}</h3><p class="mt-2 line-clamp-2 text-sm leading-6">{{ $job->description }}</p><div class="mt-5 flex items-center justify-between"><span class="text-sm font-bold text-[#6B7D83]">{{ $job->location_label ?: $job->client->city ?: 'Tu comunidad' }}</span>@if(auth()->user()?->canActAsProvider())<a class="rounded-full bg-[#123B4A] px-4 py-2 text-sm font-black text-white" href="{{ route('job-proposals.index', $job) }}">Enviar propuesta</a>@else<span class="text-xs font-black text-[#8A999E]">Solo proveedores</span>@endif</div></article>@empty<p class="col-span-full rounded-2xl border border-dashed border-[#123B4A]/20 p-8 text-center font-bold text-[#6B7D83]">No encontramos solicitudes con esos filtros.</p>@endforelse</div>@if($jobRequests->hasPages())<div class="mt-5">{{ $jobRequests->links() }}</div>@endif</section>
        @endif
    </main>
</body>
</html>
