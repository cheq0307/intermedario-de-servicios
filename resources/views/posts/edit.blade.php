...
<body class="min-h-screen bg-brand-surface text-brand-ink antialiased">
    ...
    <main class="mx-auto max-w-3xl px-5 py-9">
        ...
        <form class="mt-7 space-y-5 rounded-[2rem] border border-brand/10 bg-white p-6 shadow-sm sm:p-8" method="POST" action="{{ route('posts.update', $post) }}">
            ...
            @endif
            <label class="block"><span class="text-sm font-black">Rubro principal</span><select class="mt-2 w-full rounded-2xl bg-brand-surface px-4 py-3" name="category_id" required><option value="">Selecciona un rubro</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected((string) old('category_id', $post->category_id) === (string) $category->id)>{{ $category->name }}</option>@endforeach</select></label>
