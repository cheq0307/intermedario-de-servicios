...
class PostEditingTest extends TestCase
{
    ...
    public function test_provider_can_edit_own_product(): void
    {
        ...
        $this->actingAs($provider)->put(route('posts.update', $post), ['title' => 'Producto actualizado', 'price_type' => 'fixed', 'price' => '125.50', 'stock' => 8, 'body' => 'Descripción actualizada del producto local.'])->assertRedirect();
    ...
    public function test_client_can_edit_request_only_before_receiving_proposals(): void
    {
        ...
        $job = JobRequest::create(['public_id' => (string) Str::uuid(), 'client_id' => $client->id, 'title' => 'Solicitud original', 'description' => 'Descripción original de la solicitud.', 'status' => 'published', 'urgency' => 'normal', 'published_at' => now()]);
        $post = Post::create(['user_id' => $client->id, 'job_request_id' => $job->id, 'type' => 'job_request', 'body' => $job->description, 'published_at' => now()]);

        $this->actingAs($client)->put(route('posts.update', $post), ['title' => 'Solicitud actualizada', 'budget_min' => 200, 'budget_max' => 500, 'urgency' => 'soon', 'location_label' => 'Centro', 'body' => 'Descripción actualizada antes de recibir propuestas.'])->assertRedirect();
    ...
    private function productPost(): array
    {
        ...
        $listing = Listing::create(['vendor_id' => $vendor->id, 'type' => 'product', 'name' => 'Producto original', 'slug' => 'producto-original', 'description' => 'Descripción original del producto local.', 'price_type' => 'fixed', 'price_amount' => 10000, 'stock' => 3, 'is_active' => true]);
        $post = Post::create(['user_id' => $provider->id, 'vendor_id' => $vendor->id, 'listing_id' => $listing->id, 'type' => 'product', 'body' => $listing->description, 'published_at' => now()]);
