    ...
    <section class="mt-5 overflow-hidden rounded-[2rem] border border-[#123B4A]/10 bg-white shadow-sm">
        @forelse($conversations as $conversation)
            ...
            <a class="relative flex items-center gap-4 border-b px-5 py-4 transition last:border-0 {{ $unread ? 'bg-[#FFF8F2]' : 'hover:bg-[#FAF8F4]' }}" href="{{ route('conversations.show',$conversation) }}">
                ...
                <span class="min-w-0 flex-1"><span class="flex items-center justify-between gap-3"><strong class="truncate">{{ $support ? 'Soporte de Plaza Local' : ($other?->name ?? 'Conversación') }}</strong>@if($conversation->last_message_at)<time class="shrink-0 text-xs font-bold text-[#8A999E]">{{ $conversation->last_message_at->diffForHumans() }}</time>@endif</span><span class="mt-1 flex min-w-0 items-center gap-2">@if($support)<span class="inline-flex rounded-full bg-[#E9F7F0] px-2 py-0.5 text-[10px] font-black text-[#14734A]">CANAL OFICIAL</span>@elseif($conversation->isNegotiation())<span class="inline-flex shrink-0 rounded-full bg-[#FFF1E8] px-2 py-0.5 text-[10px] font-black text-[#C65316]">NEGOCIACIÓN</span>@elseif($conversation->isOperation())<span class="inline-flex shrink-0 rounded-full bg-[#E9F7F0] px-2 py-0.5 text-[10px] font-black text-[#14734A]">OPERACIÓN</span>@endif @if($contextTitle)<span class="truncate text-xs font-black text-[#536A72]">{{ $contextTitle }}</span>@endif</span><span class="mt-1 block truncate text-sm {{ $unread ? 'font-black text-[#123B4A]' : 'font-semibold text-[#6B7D83]' }}">{{ $latest?->body ?? 'Inicia la conversación de forma segura.' }}</span></span>
