...
@auth
    @php
        ...
        $messageBadge = $administrativeOnly ? 0 : $bottomUser->unreadConversationsCount();
    ...
    @endphp
    @if($items !== [])
        <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-brand/10 bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-bottom-navigation backdrop-blur-xl" aria-label="Navegación principal" data-activity-summary-url="{{ route('activity.summary') }}">
                ...
                @foreach($items as $item)
                    ...
                    <a class="relative flex min-h-16 flex-col items-center justify-center gap-1 px-1 text-[10px] font-black {{ $selected ? 'text-brand-danger-warm' : 'text-brand-slate' }} {{ $item['key']==='publish' ? '-translate-y-2' : '' }}" href="{{ $item['route'] }}" @if($selected) aria-current="page" @endif>
                        @switch($item['icon'])
                            ...
                            @case('messages')<span class="relative"><svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a8 8 0 0 1-8 8H5l-3 2 1-5a9 9 0 1 1 18-5Z"/></svg><span class="absolute -right-2.5 -top-2 {{ $messageBadge ? '' : 'hidden' }}" data-live-message-badge><span class="absolute inset-0 animate-ping rounded-full bg-red-400 opacity-60"></span><span class="relative block min-w-4 rounded-full bg-red-500 px-1 text-center text-[9px] leading-4 text-white" data-live-message-count>{{ $messageBadge > 99 ? '99+' : $messageBadge }}</span></span></span>@break
