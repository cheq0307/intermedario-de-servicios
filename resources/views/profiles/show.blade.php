...
<body class="min-h-screen bg-[#FAF8F4] text-[#17313A] antialiased">
    ...
    <x-market-nav :back-url="route('explore')" />

    <main class="mx-auto max-w-6xl px-5 py-8">
        @if ($isOwner)
            <div class="mb-5 flex justify-end"><a class="rounded-full bg-[#F97316] px-5 py-2.5 text-sm font-black text-white" href="{{ route('profile.edit') }}">Editar perfil</a></div>
        @endif
        @if (session('status'))
            ...
            <div class="px-6 pb-7 sm:px-9">
                ...
                @if (! $isOwner)
                    <div class="mt-7 rounded-2xl border border-[#F97316]/15 bg-[#FFF8F2] p-4">
                        @guest
                            <a class="inline-flex rounded-full bg-[#123B4A] px-6 py-3 font-black text-white" href="{{ route('register') }}">Regístrate para contactar</a>
                            <p class="mt-2 text-xs font-bold text-[#8A6A55]">Puedes explorar libremente. Solo necesitas una cuenta cuando quieras contactar, comprar o solicitar un servicio.</p>
                        @else
