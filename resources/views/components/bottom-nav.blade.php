...
@auth
    @php
        ...
        $messageBadge = $administrativeOnly ? 0 : $bottomUser->unreadNotifications()->count();
        ...
        <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-[#123B4A]/10 bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-10px_30px_rgba(18,59,74,.08)] backdrop-blur-xl" aria-label="Navegación principal">
            <div class="mx-auto grid max-w-xl grid-cols-5 px-1">
                @foreach($items as $item)
                    ...
                    <a class="relative flex min-h-16 flex-col items-center justify-center gap-1 px-1 text-[10px] font-black {{ $selected ? 'text-[#D85B0B]' : 'text-[#64787F]' }} {{ $item['key']==='publish' ? '-translate-y-2' : '' }}" href="{{ $item['route'] }}" @if($selected) aria-current="page" @endif>
                        @switch($item['icon'])
                            ...
                            @case('plus')<span class="grid size-11 place-items-center rounded-full border-4 border-white bg-[#F97316] text-white shadow-lg"><svg class="size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M12 5v14M5 12h14"/></svg></span>@break
                            ...
                            @case('messages')<span class="relative"><svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a8 8 0 0 1-8 8H5l-3 2 1-5a9 9 0 1 1 18-5Z"/></svg>@if($messageBadge)<span class="absolute -right-2.5 -top-2 min-w-4 rounded-full bg-red-500 px-1 text-center text-[9px] leading-4 text-white">{{ $messageBadge > 99 ? '99+' : $messageBadge }}</span>@endif</span>@break
