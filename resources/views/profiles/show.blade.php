...
<div class="mt-4">
<div class="flex flex-wrap items-center gap-2"><h1 class="text-3xl font-black text-[#123B4A]">{{ !$isStaff && $isProvider ? ($vendor->display_name ?: $user->name) : $user->name }}</h1>@if(!$isStaff && $vendor?->verified_at)<span class="rounded-full bg-[#E9F7F0] px-3 py-1 text-xs font-black text-[#14734A]">✓ Verificado</span>@endif</div>
