...
<div class="mt-5 grid grid-cols-2 gap-3">
    <div class="rounded-2xl border border-brand-line bg-white p-4"><span class="block text-[10px] font-black uppercase tracking-[.12em] text-brand-muted-warm">Cobertura</span><strong class="mt-1 block text-sm leading-5">{{ $user->community?->public_location_label ?: 'Por definir' }}</strong>@if($isProvider && $vendor->service_area)<span class="mt-1 block text-xs font-semibold text-brand-muted-warm">{{ $vendor->service_area }}</span>@endif</div>
