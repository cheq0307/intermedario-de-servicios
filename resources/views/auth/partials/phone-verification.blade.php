...
@if(!config('phone_verification.enabled'))
    <p class="mt-4 text-brand-copy">Guarda tu celular como dato de contacto. Por ahora no enviaremos SMS ni lo marcaremos como verificado.</p>
@elseif(!$configured)<p class="mt-4 text-brand-warning">El envío de SMS aún no está habilitado.</p>@endif
<form method="POST" action="{{ route($identity instanceof \App\Models\AdminUser ? 'admin.phone.send' : 'phone.send') }}" class="mt-5 space-y-3">
    ...
    <button class="min-h-11 rounded-xl bg-brand px-5 py-3 font-bold text-white disabled:opacity-50" @disabled(config('phone_verification.enabled') && !$configured)>{{ config('phone_verification.enabled') ? 'Enviar código por SMS' : 'Guardar celular' }}</button>
</form>
@if(config('phone_verification.enabled'))
