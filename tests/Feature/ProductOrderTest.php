...
class ProductOrderTest extends TestCase
{
    ...
    public function test_buyer_creates_order_and_inventory_is_reserved(): void
    {
        ...
        $response = $this->actingAs($buyer, 'web')->post(route('products.orders.store', $listing), [
            'quantity' => 2,
    ...
    public function test_product_order_follows_payment_and_delivery_lifecycle(): void
    {
        ...
        $this->actingAs($buyer, 'web')->post(route('products.orders.store', $listing), ['quantity' => 1]);
        $order = Order::firstOrFail();

        $this->actingAs($provider, 'web')->post(route('products.orders.simulate-payment', $order))->assertForbidden();
        $this->actingAs($buyer, 'web')->post(route('products.orders.simulate-payment', $order))->assertRedirect();
        ...
        $this->actingAs($provider, 'web')->patch(route('products.orders.ready', $order))->assertRedirect();
        $this->assertSame('ready', $order->fresh()->status->value);
        $this->actingAs($provider, 'web')->patch(route('products.orders.deliver', $order))->assertRedirect();
        $this->assertSame('delivered', $order->fresh()->status->value);
        $this->actingAs($buyer, 'web')->patch(route('orders.complete', $order))->assertRedirect();
    ...
    public function test_buyer_can_cancel_before_payment_and_stock_is_restored_once(): void
    {
        ...
        $this->actingAs($buyer, 'web')->post(route('products.orders.store', $listing), ['quantity' => 2]);
        $order = Order::firstOrFail();

        $this->actingAs($buyer, 'web')->patch(route('products.orders.cancel', $order))->assertRedirect();
        ...
        $this->actingAs($buyer, 'web')->patch(route('products.orders.cancel', $order))->assertStatus(422);
        $this->assertSame(2, $listing->fresh()->stock);
    }
    ...
    public function test_invalid_or_unsafe_product_purchases_are_rejected(): void
    {
        ...
        $this->actingAs($provider, 'web')->post(route('products.orders.store', $listing), ['quantity' => 1])->assertForbidden();
        $this->actingAs($buyer, 'web')->post(route('products.orders.store', $listing), ['quantity' => 2])->assertStatus(422);

        $listing->vendor->update(['status' => 'suspended']);
        $this->actingAs($buyer, 'web')->get(route('products.checkout', $listing))->assertStatus(422);
