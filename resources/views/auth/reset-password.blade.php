<x-layouts.auth title="Nueva contrase&ntilde;a - Plaza Local">
    <div>
        <p class="text-xs font-black uppercase tracking-[.2em] text-brand-coral-strong">Enlace verificado</p>
        <h1 class="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Crea una contrase&ntilde;a nueva</h1>
        <p class="mt-3 leading-7 text-brand-sage-muted">La contrase&ntilde;a cambia, pero tu autenticaci&oacute;n en dos pasos continuar&aacute; protegiendo la cuenta.</p>
    </div>
    ...
    <form class="mt-8 space-y-5" method="POST" action="{{ route('password.update') }}">
        ...
        <label class="block">
            ...
            <input class="mt-2 w-full rounded-2xl border border-brand-forest-deep/15 bg-white px-4 py-3.5 outline-none" type="email" name="email" value="{{ old('email', $request->email) }}" required autocomplete="email">
            @error('email') <span class="mt-2 block text-sm font-bold text-red-600">{{ $message }}</span> @enderror
        </label>
        <div class="grid gap-4 sm:grid-cols-2">
            <x-password-input id="reset-password" label="Nueva contraseña" autocomplete="new-password" minlength="8" requirements="primary" />
            <x-password-input id="reset-password-confirmation" label="Confirmar contraseña" name="password_confirmation" autocomplete="new-password" minlength="8" requirements="confirmation" />
        </div>
        ...
        <button class="w-full rounded-2xl bg-brand-coral-strong px-5 py-4 font-black text-white" type="submit">Guardar nueva contrase&ntilde;a</button>
    </form>
</x-layouts.auth>
