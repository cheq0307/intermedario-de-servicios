...
<body class="min-h-screen pb-24 bg-brand-surface text-brand-ink antialiased selection:bg-brand-orange/25">
    @php
    ...
    <main class="mx-auto grid max-w-5xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,680px)_280px]">
        <div id="inicio" class="min-w-0 space-y-5">
            <nav class="-mx-4 flex gap-6 overflow-x-auto border-b border-brand/10 px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:px-0" aria-label="Módulos del marketplace">
                @foreach($moduleLabels as $moduleKey => $moduleLabel)
                    <a class="relative shrink-0 px-0.5 pb-3 pt-1 text-sm font-bold transition {{ $module === $moduleKey ? 'text-brand' : 'text-brand-muted hover:text-brand' }}" href="{{ route('dashboard', ['module' => $moduleKey, 'feed' => $feed]) }}#actividad" @if($module === $moduleKey) aria-current="page" @endif>
                        {{ $moduleLabel }}
                        @if($module === $moduleKey)<span class="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-brand-orange"></span>@endif
                    </a>
                @endforeach
            </nav>
            @if (session('status'))
                <div class="rounded-2xl border border-brand-success-bright/20 bg-brand-success-soft px-5 py-4 text-sm font-black text-brand-success" role="status">
                    {{ session('status') }}
                </div>
            @endif
            ...
            @if($isProvider && $currentUser->vendor?->status !== 'active')
                <div class="rounded-2xl border border-brand-gold-pale bg-brand-gold-faint px-5 py-4 text-sm font-bold leading-6 text-brand-warning-copy" role="status">
            ...
            @if($feed === 'for_you' && $showcaseSections->isNotEmpty())
                <section class="space-y-7 rounded-[2rem] border border-brand/10 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="escaparate-local">
                    <div class="flex items-end justify-between gap-4">
                        <div>
                            <p class="text-xs font-black uppercase tracking-[.18em] text-brand-orange">Escaparate local</p>
                            <h2 id="escaparate-local" class="mt-1 text-2xl font-black">Descubre algo cerca de ti</h2>
                        </div>
                        <a class="shrink-0 text-sm font-black text-brand-success" href="{{ route('explore') }}">Ver todo →</a>
                    </div>
                        ...
                        <div data-market-carousel>
                            <div class="mb-3 flex items-center justify-between gap-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <a class="mr-1 hidden text-xs font-black text-brand-success sm:inline" href="{{ route('explore', ['category_id' => $section['category']->id]) }}">Ver categoría</a>
                                    <button class="hidden size-9 place-items-center rounded-full border border-brand/10 text-lg font-black transition hover:bg-brand-success-soft sm:grid" type="button" data-carousel-prev aria-label="Ver ofertas anteriores de {{ $section['category']->name }}">‹</button>
                                    <button class="hidden size-9 place-items-center rounded-full border border-brand/10 text-lg font-black transition hover:bg-brand-success-soft sm:grid" type="button" data-carousel-next aria-label="Ver más ofertas de {{ $section['category']->name }}">›</button>
                                </div>
                            </div>
                            <div id="{{ $carouselId }}" class="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" data-carousel-track tabindex="0" aria-label="Ofertas de {{ $section['category']->name }}">
                                @foreach($section['listings'] as $listing)
                                    ...
                                    @endphp
                                    <a class="group w-[72vw] max-w-[17rem] shrink-0 snap-start overflow-hidden rounded-[1.4rem] border border-brand/10 bg-brand-surface transition hover:-translate-y-1 hover:shadow-lg sm:w-[15rem]" href="{{ $destination }}">
                                        <div class="relative h-36 overflow-hidden bg-gradient-to-br from-brand-success-soft via-brand-amber-soft to-brand-gold-highlight">
                                            @if($image)
                                            ...
                                            @else
                                                <span class="grid size-full place-items-center text-sm font-black text-brand-muted">Sin imagen</span>
                                            @endif
                                            <span class="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-brand shadow-sm">{{ $listing->type->value === 'product' ? 'Producto' : 'Servicio' }}</span>
                                            @if($listing->post?->activePromotion)<span class="absolute right-3 top-3 rounded-full bg-brand-ink/90 px-2.5 py-1 text-[10px] font-black text-white shadow-sm">Patrocinado</span>@endif
                                        </div>
                                        <div class="p-4">
                                            <h4 class="line-clamp-2 min-h-12 font-black leading-6">{{ $listing->name }}</h4>
                                            <p class="mt-1 truncate text-xs font-bold text-brand-muted">{{ $listing->vendor->display_name }} · {{ $listing->vendor->user->community?->name ?? 'Tu comunidad' }}</p>
                                            <div class="mt-3 flex items-center justify-between gap-2">
                                                <strong class="truncate text-sm text-brand-success">@if($listing->price_amount === null)Cotizar @else{{ $listing->price_type->value === 'starting_at' ? 'Desde ' : '' }}${{ number_format($listing->price_amount / 100, 2) }}@endif</strong>
                                                <span class="shrink-0 rounded-full bg-brand px-3 py-1.5 text-[11px] font-black text-white">{{ $canBuy ? 'Comprar' : 'Ver' }}</span>
                                            </div>
                                        </div>
                                    </a>
                                @endforeach
            ...
            @if($publishAs === 'choose')
            <section id="publicar" class="scroll-mt-24 rounded-[1.75rem] border border-brand/10 bg-white p-5 shadow-sm">
                <p class="text-xs font-black uppercase tracking-[.18em] text-brand-orange">Crear publicación</p>
                <h2 class="mt-2 text-2xl font-black">¿Qué quieres publicar?</h2>
                <div class="mt-5 grid gap-3 sm:grid-cols-2">
                    <a class="rounded-2xl border p-5 hover:bg-brand-success-soft" href="{{ route('dashboard',['publicar'=>'offer']).'#crear-publicacion' }}"><strong class="block">Vender un producto</strong><span class="mt-1 block text-sm text-brand-copy">Comisión al concretar la venta.</span></a>
                    <a class="rounded-2xl border p-5 hover:bg-brand-success-soft" href="{{ route('dashboard',['publicar'=>'offer']).'#crear-publicacion' }}"><strong class="block">Ofrecer un servicio</strong><span class="mt-1 block text-sm text-brand-copy">Comisión al ser contratado.</span></a>
                    <a class="rounded-2xl border p-5 hover:bg-brand-orange-soft" href="{{ route('dashboard',['publicar'=>'request']).'#crear-publicacion' }}"><strong class="block">Solicitar algo</strong><span class="mt-1 block text-sm text-brand-copy">Pide un producto o servicio.</span></a>
                    <a class="rounded-2xl border border-brand-gold/30 bg-brand-warning-faint p-5" href="{{ route('vacancies.create') }}"><strong class="block text-brand-warning-deep">Publicar vacante de empleo</strong><span class="mt-1 block text-sm text-brand-warning-copy">La tarifa se muestra antes del pago.</span></a>
                </div>
            </section>
            @endif
            @if($showComposer)
            <section id="crear-publicacion" class="scroll-mt-24 rounded-[1.75rem] border border-brand/10 bg-white p-5 shadow-sm sm:p-6">
                <div class="flex items-center gap-3">
                    <span class="grid size-11 place-items-center rounded-full bg-brand-avatar-strong font-black text-brand">{{ mb_strtoupper(mb_substr($currentUser->name, 0, 1)) }}</span>
                    <div>
                        <h2 class="font-black">Crea una publicación</h2>
                        <p class="text-xs font-semibold text-brand-muted">Aparecerá en la actividad de tu comunidad.</p>
                    </div>
                </div>
                ...
                <form class="mt-5 space-y-4" method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data" data-publication-form>
                    ...
                    @if ($isProvider)
                        <div class="grid gap-4 sm:grid-cols-2" data-listing-fields>
                            <label class="block sm:col-span-2">
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50 focus:ring-4 focus:ring-brand-orange/10" type="text" name="title" value="{{ old('title') }}" maxlength="120" placeholder="Ej. Instalación eléctrica o Tacos al pastor" data-listing-required>
                            ...
                            <label class="block">
                                ...
                                <select class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" name="price_type" data-price-type data-listing-required>
                            ...
                            <label class="block" data-price-field>
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" type="number" name="price" value="{{ old('price') }}" min="0" step="0.01" inputmode="decimal" placeholder="0.00" data-price-input>
                            ...
                            <label class="block" data-product-field>
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" type="number" name="stock" value="{{ old('stock') }}" min="0" step="1" inputmode="numeric" placeholder="Opcional">
                    ...
                    @else
                        <div class="grid gap-4 sm:grid-cols-2">
                            <label class="block sm:col-span-2">
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50 focus:ring-4 focus:ring-brand-orange/10" type="text" name="title" value="{{ old('title') }}" minlength="5" maxlength="120" required placeholder="Ej. Busco plomero para reparar una fuga">
                                @error('title') <span class="mt-1 block text-sm font-bold text-red-600">{{ $message }}</span> @enderror
                            </label>

                            <label class="block">
                                <span class="text-sm font-black">Presupuesto mínimo</span>
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" type="number" name="budget_min" value="{{ old('budget_min') }}" min="0" step="0.01" inputmode="decimal" placeholder="$ MXN">
                            ...
                            <label class="block">
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" type="number" name="budget_max" value="{{ old('budget_max') }}" min="0" step="0.01" inputmode="decimal" placeholder="$ MXN">
                            ...
                            <label class="block">
                                ...
                                <select class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" name="urgency" required>
                            ...
                            <label class="block">
                                ...
                                <input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold outline-none focus:border-brand-orange/50" type="text" name="location_label" value="{{ old('location_label') }}" maxlength="120" placeholder="Colonia, barrio o referencia">
                    ...
                    @endunless
                    <textarea class="min-h-28 w-full resize-y rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3 text-sm font-semibold leading-6 outline-none transition placeholder:text-brand-caption focus:border-brand-orange/50 focus:ring-4 focus:ring-brand-orange/10" name="body" maxlength="1500" required placeholder="{{ $isProvider ? 'Describe lo que ofreces, disponibilidad, entrega y zona de atención…' : 'Explica los detalles necesarios para que los proveedores puedan responderte…' }}">{{ old('body') }}</textarea>
                    @error('body') <p class="text-sm font-bold text-red-600">{{ $message }}</p> @enderror
                    <label class="block rounded-2xl border border-dashed border-brand/20 bg-brand-surface p-4">
                        <span class="text-sm font-black">Fotos o videos</span>
                        <span class="mt-1 block text-xs font-semibold text-brand-muted">Hasta 6 archivos. Maximo 50 MB por archivo.</span>
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <p class="text-xs font-semibold text-brand-caption">La ubicación exacta nunca se mostrará públicamente.</p>
                        <button class="shrink-0 rounded-full bg-brand-orange px-5 py-2.5 text-sm font-black text-white transition hover:bg-brand-orange-vivid disabled:cursor-wait disabled:opacity-60" type="submit" data-submit-button>Publicar</button>
                    </div>
                </form>
            </section>
            ...
            <section id="actividad" class="scroll-mt-24 space-y-4">
                ...
                @forelse ($posts as $post)
                    <article id="post-{{ $post->id }}" class="scroll-mt-24 overflow-hidden rounded-[1.5rem] border border-brand/10 bg-white shadow-feed-card">
                        <div class="p-4 sm:p-5">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex min-w-0 items-center gap-3">
                                    <a class="grid size-10 shrink-0 place-items-center overflow-hidden rounded-full bg-brand-avatar-strong font-black text-brand" href="{{ route('profile.show', $post->user) }}" aria-label="Ver perfil de {{ $post->user->name }}">
                                ...
                                <details class="relative shrink-0">
                                    <summary class="grid size-9 cursor-pointer list-none place-items-center rounded-full text-xl leading-none text-brand-muted hover:bg-brand-neutral-canvas" aria-label="Opciones de la publicación">···</summary>
                                    <div class="absolute right-0 top-10 z-20 w-44 rounded-2xl border border-brand/10 bg-white p-2 shadow-xl">
                                        <a class="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-brand-surface" href="{{ route('profile.show', $post->user) }}">Ver perfil</a>
                                        @if($post->user_id === $currentUser->id)<a class="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-brand-surface" href="{{ route('posts.edit', $post) }}">Editar publicación</a>@endif
                                    </div>
                                </details>
                            </div>
                            ...
                            @if ($post->listing)
                                <div class="mt-4 border-t border-brand/10 pt-4">
                                    ...
                                    @if ($post->type === 'product' && $post->listing->stock !== null)<p class="mt-1 text-xs font-bold text-brand-muted">{{ $post->listing->stock }} unidades disponibles</p>@endif
                                </div>
                            @elseif ($post->jobRequest)
                                ...
                                <div class="mt-4 border-t border-brand/10 pt-4">
                                    ...
                                    @endif
                                    <p class="mt-1 text-xs font-semibold text-brand-muted">{{ $urgencyLabels[$post->jobRequest->urgency] ?? 'Sin prisa' }}@if($post->jobRequest->location_label) · {{ $post->jobRequest->location_label }}@endif</p>
                                </div>
                            @endif
                        ...
                        @endphp
                        <div class="px-4 {{ $hasPrimaryAction ? 'border-t border-brand/8 pt-2' : '' }}">
                            @if($hasPrimaryAction)
                            <div class="flex min-h-11 items-center justify-end text-xs font-black text-brand-copy">
                            @if ($ownsPost)
                                @if ($post->jobRequest)
                                    <a class="rounded-xl px-3 py-2.5 text-center transition hover:bg-brand-surface hover:text-brand-orange" href="{{ route('job-proposals.index', $post->jobRequest) }}">Ver propuestas</a>
                                @else
                                    <a class="rounded-xl px-3 py-2.5 text-center transition hover:bg-brand-surface hover:text-brand-orange" href="{{ route('posts.edit', $post) }}">Administrar</a>
                                @endif
                            @else
                                @if ($canSendProposal)
                                    <a class="rounded-xl px-3 py-2.5 text-center transition hover:bg-brand-surface hover:text-brand-orange" href="{{ route('job-proposals.index', $post->jobRequest) }}">Enviar propuesta</a>
                                @elseif ($canBuyProduct)
                                    <a class="rounded-xl px-3 py-2.5 text-center transition hover:bg-brand-surface hover:text-brand-orange" href="{{ route('products.checkout', $post->listing) }}">Comprar</a>
                                @elseif ($canContactSeller)
                                    <form method="POST" action="{{ route('posts.conversations.start', $post) }}">
                                        @csrf
                                        <button class="rounded-xl px-3 py-2.5 transition hover:bg-brand-surface hover:text-brand-orange" type="submit">{{ $post->listing?->price_type?->value === 'quote' ? 'Solicitar cotización' : 'Me interesa' }}</button>
                                    </form>
                                @endif
                            @endif
                            </div>
                            @endif
                            <div class="flex items-center gap-1 border-t border-brand/8 py-2 text-brand">
                                <form method="POST" action="{{ route('posts.reactions.toggle', $post) }}">
                                    @csrf
                                    <button class="group flex min-h-11 items-center gap-2 rounded-full px-3 transition hover:bg-brand-orange-soft hover:text-brand-danger-warm {{ $post->reacted_by_user ? 'text-brand-danger-coral' : '' }}" type="submit" aria-pressed="{{ $post->reacted_by_user ? 'true' : 'false' }}" title="{{ $post->reacted_by_user ? 'Retirar Me gusta' : 'Marcar con Me gusta' }}">
                                ...
                                <button class="group flex min-h-11 items-center gap-2 rounded-full px-3 transition hover:bg-brand-success-soft hover:text-brand-success" type="button" data-comment-toggle="comment-{{ $post->id }}" title="Comentar">
                                ...
                                <form method="POST" action="{{ route('posts.shares.store', $post) }}" data-share-form data-share-url="{{ route('dashboard').'#post-'.$post->id }}" data-share-title="{{ $post->user->name }} en Plaza Local">
                                    ...
                                    <button class="group flex min-h-11 items-center gap-2 rounded-full px-3 transition hover:bg-brand-info-soft hover:text-brand" type="submit" title="Compartir">
                                ...
                                @if(! $ownsPost)
                                    <form class="ml-auto" method="POST" action="{{ route('posts.conversations.start', $post) }}">
                                        @csrf
                                        <button class="group flex min-h-11 items-center gap-2 rounded-full px-3 transition hover:bg-brand-success-soft hover:text-brand-success" type="submit" title="Abrir chat sobre esta publicación">
                        ...
                        <section id="comment-{{ $post->id }}" class="border-t border-brand/8 bg-brand-surface/60 p-4" data-comment-panel>
                            ...
                            @if($post->comments_count > $post->comments->count())
                                <button class="mb-3 block text-sm font-black text-brand-success hover:underline disabled:cursor-wait disabled:opacity-60" type="button" data-comments-load data-comments-url="{{ route('posts.comments.index', $post) }}" data-comments-total="{{ $post->comments_count }}">Ver los {{ $post->comments_count }} comentarios</button>
                            @endif
                            @if($post->comments_enabled)<form class="flex gap-2" method="POST" action="{{ route('posts.comments.store', $post) }}">@csrf<input class="min-w-0 flex-1 rounded-full border border-brand/10 bg-white px-4 py-2.5 text-sm outline-none" name="body" maxlength="1000" required placeholder="Escribe un comentario"><button class="rounded-full bg-brand px-4 py-2 text-xs font-black text-white" type="submit">Publicar</button></form>@endif
                        </section>
                    </article>
                @empty
                    <div class="rounded-[1.75rem] border border-dashed border-brand/20 bg-white/60 px-6 py-12 text-center">
                        <span class="mx-auto grid size-14 place-items-center rounded-2xl bg-brand-orange-soft text-2xl font-black text-brand-orange">+</span>
                        <h3 class="mt-4 text-lg font-black">Sé la primera publicación</h3>
                        <p class="mx-auto mt-2 max-w-sm text-sm leading-6 text-brand-muted">No hay publicaciones en esta sección todavía. Puedes explorar otra pestaña o crear la primera.</p>
                    </div>
                @endforelse
        ...
        <aside class="hidden xl:block">
            <div class="sticky top-24 space-y-4">
                <section class="rounded-[1.75rem] border border-brand/10 bg-white p-5 shadow-sm">
                    <div class="flex items-center justify-between">
                        <h2 class="font-black">Explora por categoría</h2>
                        <span class="text-xs font-black text-brand-orange">Ver todo</span>
                    </div>
                    <div class="mt-4 space-y-2">
                        @foreach (['Comida local', 'Hogar y reparaciones', 'Productos y tiendas', 'Belleza y cuidado', 'Transporte'] as $category)
                            <button class="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm font-bold text-brand-copy transition hover:bg-brand-surface hover:text-brand" type="button">
                                {{ $category }} <span class="text-brand-disabled">›</span>
                            </button>
                        @endforeach
                    </div>
                </section>

                <section id="proximamente" class="rounded-[1.75rem] bg-brand-gold-pale p-5">
                    <p class="text-xs font-black uppercase tracking-[.16em] text-brand-amber-deep">Compra protegida</p>
                    <h2 class="mt-2 text-lg font-black">Acuerdos dentro de Plaza Local</h2>
                    <p class="mt-2 text-sm font-semibold leading-6 text-brand-earth-strong">Las propuestas, pagos de prueba, entregas y disputas quedan registradas para proteger a ambas partes.</p>
                </section>
            </div>
        </aside>
    </main>
