        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
            ...
            <section class="rounded-[2rem] border border-brand/10 bg-white p-5 shadow-sm sm:p-8">
                ...
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <label class="block"><span class="text-sm font-black">Teléfono celular privado</span><input class="mt-2 w-full scroll-mt-24 rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3" type="tel" name="phone" value="{{ old('phone', $user->phone) }}" inputmode="numeric" pattern="[0-9]{10}" minlength="10" maxlength="10" autocomplete="tel-national" placeholder="10 dígitos">@error('phone')<span class="mt-1 block text-sm font-bold text-red-600">{{ $message }}</span>@enderror<span class="mt-1 block text-xs font-bold text-brand-caption">Exactamente 10 dígitos; no aparecerá en tu perfil público.</span></label>
