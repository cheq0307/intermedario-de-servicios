    ...
    <section class="mt-5 overflow-hidden rounded-[2rem] border border-[#123B4A]/10 bg-white shadow-sm">
        @forelse($conversations as $conversation)
            ...
            <a class="relative flex items-center gap-4 border-b px-5 py-4 transition last:border-0 {{ $unread ? 'bg-[#FFF8F2]' : 'hover:bg-[#FAF8F4]' }}" href="{{ route('conversations.show',$conversation) }}">
                @if($other?->avatar_path)<img class="size-14 rounded-full object-cover" src="{{ asset('storage/'.$other->avatar_path) }}" alt="">@else<span class="grid size-14 shrink-0 place-items-center rounded-full {{ $support ? 'bg-[#123B4A] text-white' : 'bg-[#DCEAE6] text-[#123B4A]' }} text-lg font-black">{{ $other ? mb_strtoupper(mb_substr($other->name,0,1)) : '?' }}</span>@endif
