        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
            ...
            <section class="rounded-[2rem] border border-[#123B4A]/10 bg-white p-6 shadow-sm sm:p-8">
                ...
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <div data-postal-assistant><label class="block"><span class="text-sm font-black">Código postal</span><span class="mt-2 flex gap-2"><input class="min-w-0 flex-1 rounded-2xl border border-[#123B4A]/10 bg-[#FAF8F4] px-4 py-3" data-postal-input value="{{ old('postal_code', $user->community?->postal_code) }}" inputmode="numeric" pattern="[0-9]{5}" maxlength="5" placeholder="5 dígitos"><button class="rounded-2xl bg-[#123B4A] px-4 text-xs font-black text-white" data-postal-submit type="button">Buscar</button></span></label><p class="mt-1 text-xs font-bold text-[#8A999E]" data-postal-status aria-live="polite">Úsalo para localizar tu municipio y comunidad.</p></div>
                    <label class="block"><span class="text-sm font-black">Ciudad y comunidad</span><select class="mt-2 w-full rounded-2xl border border-[#123B4A]/10 bg-[#FAF8F4] px-4 py-3" name="community_id" data-community-select required><option value="">Selecciona tu comunidad</option>@foreach($communities as $community)<option value="{{ $community->id }}" data-postal-code="{{ $community->postal_code }}" @selected((string) old('community_id', $user->community_id) === (string) $community->id)>{{ $community->display_label }}</option>@endforeach</select>@error('community_id')<span class="mt-1 block text-sm font-bold text-red-600">{{ $message }}</span>@enderror<span class="mt-1 block text-xs font-bold text-[#8A999E]">Solo aparecen comunidades habilitadas por administración.</span></label>
            ...
            @if ($isProvider)
                <section id="servicios" class="scroll-mt-24 rounded-[2rem] border border-[#123B4A]/10 bg-white p-6 shadow-sm sm:p-8">
                    ...
                    <p class="mt-2 text-sm font-semibold text-[#6B7D83]">Esta selección es distinta de tus intereses: aquí indicas lo que otras personas pueden contratarte o comprarte.</p>
