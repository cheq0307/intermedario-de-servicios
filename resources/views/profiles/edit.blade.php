...
<body class="min-h-screen bg-brand-surface text-brand-ink antialiased">
    ...
    <main class="mx-auto max-w-4xl px-5 py-9">
        ...
        @endif
        <section class="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-[2rem] border border-brand/10 bg-white p-6 shadow-sm"><div><p class="text-xs font-black uppercase tracking-[.16em] text-brand-orange">Cuenta unificada</p><h2 class="mt-2 text-xl font-black">Eres usuario de Plaza Local</h2><p class="mt-2 text-sm font-semibold text-brand-muted">Puedes solicitar y comprar. Si quieres ofrecer, crea y completa la información comercial en esta misma cuenta.</p></div>@if(! $profileForm->hasCommercialProfile)<form method="POST" action="{{ route('capabilities.activate', $profileForm->providerCapability) }}">@csrf<button class="rounded-full bg-brand px-5 py-3 text-sm font-black text-white" type="submit">Agregar mis servicios</button></form>@else<span class="rounded-full bg-brand-success-soft px-4 py-2 text-xs font-black text-brand-success">{{ $profileForm->vendorStatusLabel }}</span>@endif</section>
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
            ...
            <section class="rounded-[2rem] border border-brand/10 bg-white p-6 shadow-sm sm:p-8">
                ...
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <div data-postal-assistant><label class="block"><span class="text-sm font-black">Código postal</span><span class="mt-2 flex gap-2"><input class="min-w-0 flex-1 rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3" data-postal-input value="{{ old('postal_code', $user->community?->postal_code) }}" inputmode="numeric" pattern="[0-9]{5}" maxlength="5" placeholder="5 dígitos"><button class="rounded-2xl bg-brand px-4 text-xs font-black text-white" data-postal-submit type="button">Buscar</button></span></label><p class="mt-1 text-xs font-bold text-brand-caption" data-postal-status aria-live="polite">Úsalo para localizar tu municipio y comunidad.</p></div>
                    ...
                    <label class="block"><span class="text-sm font-black">Teléfono celular privado</span><input class="mt-2 w-full rounded-2xl border border-brand/10 bg-brand-surface px-4 py-3" type="tel" name="phone" value="{{ old('phone', $user->phone) }}" inputmode="numeric" pattern="[0-9]{10}" minlength="10" maxlength="10" autocomplete="tel-national" placeholder="10 dígitos">@error('phone')<span class="mt-1 block text-sm font-bold text-red-600">{{ $message }}</span>@enderror<span class="mt-1 block text-xs font-bold text-brand-caption">Exactamente 10 dígitos; no aparecerá en tu perfil público.</span></label>
            ...
            <div class="flex flex-wrap justify-end gap-3"><a class="rounded-full border border-brand/10 bg-white px-7 py-3.5 font-black" href="{{ route('more.index') }}">Cancelar</a><button class="rounded-full bg-brand-orange px-7 py-3.5 font-black text-white shadow-lg shadow-brand-orange/15" type="submit">Guardar perfil</button></div>
        </form>
        @if($profileForm->hasCommercialProfile)
